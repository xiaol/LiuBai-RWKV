...
def parser():
    ...
    for name in ("doctor", "generate", "batch"):
        ...
        q.add_argument("--vae", default="standard", help="standard (old), YuE2-Vae-legacy, local path or HF repo")
    ...
    for name in ("verify", "bench", "eval"):
        q = sub.add_parser(name, add_help=False)
        q.add_argument("arguments", nargs=argparse.REMAINDER)
    return p


def main(argv=None):
    argv = sys.argv[1:] if argv is None else argv
    if argv and argv[0] in {"verify", "bench", "eval"}:
        import subprocess
        script = {"verify": "validate_release.py", "bench": "benchmark.py", "eval": "evaluate.py"}[argv[0]]
        target = kit_root() / "scripts" / script
        if not target.is_file():
            raise FileNotFoundError("This command needs the complete minimal kit (scripts/data/evaluation_runtime). "
                                    "Set YUE2_KIT to its directory; the HF wheel supplies generation and the Python API.")
